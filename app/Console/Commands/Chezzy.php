...
class Chezzy extends Command
{
    ...
    public function handle()
    {
        ...
        $tasks = [
            ...
            'Menginstall Composer dependencies' => fn() => $this->runProcess(['composer', 'install', '--no-interaction', '--prefer-dist', '--optimize-autoloader']),
            'Menginstall NPM dependencies & build' => fn() => $this->runProcess(['npm', 'install', '&&', 'npm', 'run', 'build']),
            'Menjalankan migrasi database' => fn() => $this->callSilent('migrate', ['--force' => true]),
            'Menjalankan seeder' => fn() => $this->callSilent('db:seed', ['--class' => 'UserRolePermissionSeeder']),
        ];

        $messages = [
            'Menyalin .env & generate key' => ['<fg=cyan>Berhasil Menyalin .env & generate key.</>', '<fg=yellow>.env & Key sudah ada, tidak perlu diubah.</>'],
            'Menginstall Composer dependencies' => ['<fg=cyan>Berhasil Menginstall Composer dependencies.</>', '<fg=yellow>Composer dependencies sudah terinstall sebelumnya.</>'],
            ...
            'Menjalankan migrasi database' => ['<fg=cyan>Berhasil Menjalankan migrasi database.</>', '<fg=yellow>Database telah dimigrasi sebelumnya.</>'],
            'Menjalankan seeder' => ['<fg=cyan>Berhasil Menjalankan seeder.</>', '<fg=yellow>Seeder telah dijalankan sebelumnya.</>'],
        ];
